	...
	async fetch(request: Request, env: Env): Promise<Response> {
		...
		if (url.pathname === PATHAPI) {
			...
			if (processedLinks.length > 0) {
				await Promise.all(processedLinks.map((link) => sendMessage(TELEGRAM_API, chatId, link)));
			} else {
				// Only send report if OWNER_ID is set AND the message is NOT from the owner (prevent loop)
				const isOwner = env.OWNER_ID && chatId.toString() === env.OWNER_ID;

				if (env.OWNER_ID && !isOwner) {
					await sendMessage(TELEGRAM_API, env.OWNER_ID, `Unprocessed message from ${senderName} (chat ${chatId}):\n\n${text}`);
				}

				if (env.UNPROCESSED_KV) {
					const key = `${Date.now()}-${chatId}`;
					await env.UNPROCESSED_KV.put(
						key,
						JSON.stringify({
							chatId,
							text,
